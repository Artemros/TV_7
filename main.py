...
def table(sel, k, file):
    ...
    file.write(r'\hline $i$ & $\Delta_i$ & $n_i$ & $p_i$ & $np_i$ & $n_i-np_i$ & $\frac{(n_i-np_i)^2}{np_i}$\\' + '\n')
    x = np.array(sel)
    x.sort()
    d = (x[-1] - x[0]) / k
    F = norm_mle_cdf(x)
    ...
    for i in range(k):
        ...
        s += f'{tr(n * p, 2)} & {tr(ni - n * p, 2)} & '
        dchi = round((ni - n * p) ** 2 / (n * p), 2)
        s += f'{tr(dchi, 4)} ' + r'\\'
        chi += dchi
        file.write(s + '\n')
    chi = round(chi, 2)
    file.write(f'\\hline Sum & - & {n} & 1.0000 & {n}.00 & 0.00 & ' + tr(chi, 2) + ' \\\\\n')
    file.write('\\hline\n\\end{tabular}\n')
